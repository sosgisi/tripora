...
export default function BookingForm({ trip }: { trip: Trip }) {
    ...
    useEffect(() => {
        ...
        async function fetchTrip() {
            try {
                ...
                console.error('فشل في جلب بيانات الرحلة: ', err);
            }
        }
        fetchUser();
        fetchTrip();
    ...
    return (
        <div>
            <h1 className="mt-5 font-bold text-lg">معلومات المشارك الرئيسي</h1>
            ...
            <div>
                <h1 className="mt-3">اختر عدد الأشخاص</h1>
                <select
                    name="participants"
                    id="participants"
            ...
            <div className="flex items-center gap-2 mt-5">
                ...
                <label htmlFor="terms">
                    أوافق على <a href="/terms" className="text-primary dark:text-primary-dark underline">الشروط والأحكام</a>
                </label>
            </div>
            <div className="flex items-center gap-2 mt-5">
                <input type="checkbox" onChange={(e) => setFormData({ ...formData, cancellationPolicyAccepted: e.target.checked })} />
                <label htmlFor="terms">أفهم سياسة الإلغاء واسترداد الأموال لهذه الرحلة</label>
            </div>
        </div>
    )
}
