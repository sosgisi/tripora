...
export default function DetailTripTabs({trip}: {trip: Trip|null}) {
    ...
    return(
        <>
        <div className="grid grid-cols-4 w-full p-0.5 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 mt-10" dir="rtl">
            <h1 
                onClick={() => setOpenTab('description')} 
                className={`text-center text-white font-bold py-2 rounded-xl cursor-pointer ${openTab==='description' ? 'bg-primary-hover' : 'text-gray-500!'}`}>
                الوصف
            </h1>
            <h1 
                onClick={() => setOpenTab('itinerary')} 
                className={`text-center text-white font-bold py-2 rounded-xl cursor-pointer ${openTab==='itinerary' ? 'bg-primary-hover' : 'text-gray-500!'}`}>
                البرنامج
            </h1>
            <h1 
                onClick={() => setOpenTab('include')} 
                className={`text-center text-white font-bold py-2 rounded-xl cursor-pointer ${openTab==='include' ? 'bg-primary-hover' : 'text-gray-500!'}`}>
                يشمل
            </h1>
            <h1 
                onClick={() => setOpenTab('review')} 
                className={`text-center text-white font-bold py-2 rounded-xl cursor-pointer ${openTab==='review' ? 'bg-primary-hover' : 'text-gray-500!'}`}>
                التقييمات
            </h1>
        </div>
        ...
        {/* محتوى الوصف */}
        { openTab === 'description' &&
            <div className="mt-10 p-10 bg-white dark:bg-gray-800 rounded-md border border-gray-200 text-gray-500" dir="rtl">
        ...
        { openTab === 'include' &&
            <div className="mt-10 p-10 bg-white dark:bg-gray-800 rounded-md border border-gray-200 text-gray-500" dir="rtl">
                تفاصيل ما يشمله العرض ستظهر هنا.
            </div>
        }
